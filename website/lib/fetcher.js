...
Fetcher.getStatsForAccount = function(account,callback){
  client.keys(account.accountNumber+'_*',function(err,replies){
    if(!err){
      console.log(replies);
    }
    else {
      logger.error("Lib/Fetcher | Failed to get keys from Redis for "+account.accountName+"("+account.accountNumber+")");
      callback(err);
    }
  });
}

Fetcher.getSummaryForCategoryForAll = function(category,callback){
...
Fetcher.getStatusCountsForAccount = function(account,callback){
...
Fetcher.getStatusCountsForAll = function(callback){
  myAccounts.scan({},function(err,accounts){
    if(!err){
      ...
      accounts.forEach(function(account){
        calls.push(function(hollaback){
          Fetcher.getStatusCountsForAccount(account,function(err,res){
            if(!err){
 ...
 */
Fetcher.fetchStatsForAll = function(callback) {
    mAccounts.scan({}, function(err, accounts) {
        if (!err) {
            accounts.forEach(function(account) {
